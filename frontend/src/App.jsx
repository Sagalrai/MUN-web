...
function Sidebar({ open, onClose, onLogout }) {
  const navItems = [{ label: 'Dashboard', to: '/', icon: LayoutDashboard }, { label: 'Delegates', to: '/delegates', icon: Users }, { label: 'OC Management', to: '/volunteers', icon: ClipboardList }, { label: 'Orientation Reports', to: '/orientation-reports', icon: ClipboardCheck }, { label: 'Purchase Reports', to: '/purchase-reports', icon: ReceiptIcon }, { label: 'OC Workspace', to: '/oc-workspace', icon: CalendarDays }, { label: 'Import data', to: '/import', icon: FileUp }]
...
function Login({ onLogin }) { const [kind, setKind] = useState('admin'); const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [error, setError] = useState(''); const submit = async (e) => { e.preventDefault(); try { const result = await api(kind === 'admin' ? '/auth/login' : '/auth/oc-login', { method: 'POST', body: JSON.stringify({ email, password }) }); localStorage.setItem('qrmun_token', result.token); onLogin(result.admin || result.user) } catch (err) { setError(err.message) } }; return <main className="login-page"><div className="login-card"><div className="login-brand"><Crest /><strong>QRMUN</strong></div><p className="eyebrow">{kind === 'admin' ? 'Administration' : 'OC workspace'}</p><h1>Welcome back.</h1><p>Sign in to manage conference reporting.</p><div className="login-switch"><button className={kind === 'admin' ? 'active' : ''} onClick={() => setKind('admin')} type="button">Admin</button><button className={kind === 'oc' ? 'active' : ''} onClick={() => setKind('oc')} type="button">OC</button></div><form onSubmit={submit}><label>Email<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required /></label><label>Password<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required /><small>OC access is provisioned from OC Management.</small></label><ErrorMessage message={error} /><Button type="submit">Sign in</Button></form></div><Signature /></main> }
...
function OrientationComposer({ user, notify }) {
  ...
  const current = reports[0]; const locked = current && ['Pending Review', 'Approved', 'Attendance Pending', 'Finalized'].includes(current.reviewStatus);
  return <PageFrame><SectionHeading eyebrow="OC workspace" title="Orientation" description="Submit the official expected list, then complete the approved attendance sheet." /><ErrorMessage message={error} />{current && <section className="surface report-status"><SurfaceHeading eyebrow="Latest submission" title={current.event} /><p><StatusPill status={current.reviewStatus} /> {current.reviewStatus === 'Pending Review' ? 'Pending Admin Review' : current.reviewStatus === 'Approved' ? 'Approved by Admin · Attendance Pending' : current.reviewStatus === 'Finalized' ? 'Final Attendance Submitted' : current.reviewStatus}</p><div className="selected-members">{current.expectedMembers.map((member) => <span key={member._id}>{member.name} ✓</span>)}</div>{current.reviewStatus === 'Approved' && <AttendanceForm report={current} notify={notify} onDone={load} />}</section>}{!locked && <section className="surface"><SurfaceHeading eyebrow="Before orientation" title="Expected attendance" /><form onSubmit={submit} className="report-form"><label>Event<input required value={form.event} onChange={(e) => setForm({ ...form, event: e.target.value })} /></label><label>Orientation date<input required type="date" value={form.orientationDate} onChange={(e) => setForm({ ...form, orientationDate: e.target.value })} /></label><div className="member-picker">{members.map((member) => <label key={member._id} className="check-row"><input type="checkbox" checked={selected.includes(member._id)} onChange={() => toggle(member._id)} /><span>{member.name}<small>{member.volunteerId} · {member.role}</small></span></label>)}</div><Button type="submit" disabled={!selected.length}><Check size={16} />Submit expected list</Button></form></section>}<section className="surface"><SurfaceHeading eyebrow="Submission history" title="Previous reports" />{reports.length > 1 ? reports.slice(1).map((report) => <div className="activity-item" key={report._id}><div><strong>{report.event}</strong><small>{new Date(report.createdAt).toLocaleString()} · {report.expectedMembers.length} expected</small></div><StatusPill status={report.reviewStatus} /></div>) : <p className="heading-description">No previous submissions.</p>}</section></PageFrame>
}
function AttendanceForm({ report, notify, onDone }) { const [values, setValues] = useState(Object.fromEntries(report.expectedMembers.map((member) => [member._id, 'Present']))); const [error, setError] = useState(''); const submit = async (event) => { event.preventDefault(); try { await api(`/reports/orientation/${report._id}/finalize`, { method: 'POST', body: JSON.stringify({ attendance: Object.entries(values).map(([member, status]) => ({ member, status })) }) }); notify('Final attendance submitted'); onDone() } catch (e) { setError(e.message) } }; return <form onSubmit={submit} className="attendance-form"><h3>Actual Attendance</h3>{report.expectedMembers.map((member) => <label key={member._id}>{member.name}<select value={values[member._id]} onChange={(e) => setValues({ ...values, [member._id]: e.target.value })}><option>Present</option><option>Absent</option></select></label>)}<ErrorMessage message={error} /><Button type="submit">Submit final attendance</Button></form> }
function OrientationReports({ notify }) { const [reports, setReports] = useState([]); const [error, setError] = useState(''); const load = () => api('/reports/orientation').then(setReports).catch((e) => setError(e.message)); useEffect(load, []); const review = async (report, status) => { const reason = status === 'Returned' ? window.prompt('Optional return reason') || '' : ''; try { await api(`/reports/orientation/${report._id}/review`, { method: 'PATCH', body: JSON.stringify({ status, reason }) }); notify(`Report ${status.toLowerCase()}`); load() } catch (e) { setError(e.message) } }; return <PageFrame><SectionHeading eyebrow="Admin Panel" title="Orientation Reports" description="Review expected attendance and inspect final attendance without overwriting the original list." /><ErrorMessage message={error} /><section className="table-surface"><div className="table-meta"><span>Submitted reports</span><span>{reports.length} records</span></div><div className="table-wrap"><table><thead><tr><th>Submitter</th><th>Event / date</th><th>Expected</th><th>Actual</th><th>Status</th><th /></tr></thead><tbody>{reports.map((report) => { const absent = report.actualAttendance?.filter((entry) => entry.status === 'Absent') || []; const expectedMembers = Array.isArray(report.expectedMembers) ? report.expectedMembers : []; return <tr key={report._id}><td><strong>{report.submittedBy?.name}</strong><small>{new Date(report.createdAt).toLocaleString()}</small></td><td>{report.event}<br /><small>{new Date(report.orientationDate).toLocaleDateString()}</small></td><td><strong>Expected attendees: {expectedMembers.length}</strong><div className="expected-members"><small>Expected attendees</small>{expectedMembers.length ? <ul>{expectedMembers.map((member, index) => <li key={member?._id || index}>{member?.name || 'Member unavailable'}</li>)}</ul> : <span>No expected attendees</span>}</div></td><td>{report.actualAttendance?.length ? `${report.actualAttendance.filter((entry) => entry.status === 'Present').length} present · ${absent.length} absent` : 'Attendance Pending'}{absent.length > 0 && <small className="absent-names">Absent: {absent.map((entry) => entry.member?.name).join(', ')}</small>}</td><td><StatusPill status={report.reviewStatus} /></td><td>{report.reviewStatus === 'Pending Review' && <div className="row-actions"><button className="row-action" title="Approve" onClick={() => review(report, 'Approved')}><Check size={16} /></button><button className="row-action danger" title="Return" onClick={() => review(report, 'Returned')}><X size={16} /></button></div>}</td></tr>})}</tbody></table></div></section></PageFrame> }
function PurchaseReports() { const [expenses, setExpenses] = useState([]); const [preview, setPreview] = useState(null); const [error, setError] = useState(''); useEffect(() => { api('/reports/expenses').then(setExpenses).catch((e) => setError(e.message)) }, []); const openReceipt = async (expense) => { try { const response = await fetch(`${API_URL}/reports/expenses/${expense._id}/receipt`, { headers: { Authorization: `Bearer ${localStorage.getItem('qrmun_token')}` } }); if (!response.ok) throw new Error('Unable to open receipt'); setPreview(URL.createObjectURL(await response.blob())) } catch (e) { setError(e.message) } }; return <PageFrame><SectionHeading eyebrow="Admin Panel" title="Purchase Reports" description="Review submitted purchases and inspect receipt images in place." /><ErrorMessage message={error} /><section className="table-surface"><div className="table-meta"><span>Purchases</span><span>{expenses.length} records</span></div><div className="table-wrap"><table><thead><tr><th>OC</th><th>Item</th><th>Quantity</th><th>Amount</th><th>Date</th><th>Purpose</th><th>Receipt</th></tr></thead><tbody>{expenses.map((expense) => <tr key={expense._id}><td>{expense.submittedBy?.name}</td><td>{expense.item}</td><td>{expense.quantity}</td><td>Rs. {Number(expense.totalCost).toLocaleString()}</td><td>{new Date(expense.purchaseDate).toLocaleDateString()}</td><td>{expense.purpose}</td><td><button className="row-action" title="Preview receipt" onClick={() => openReceipt(expense)}><Eye size={16} /></button></td></tr>)}</tbody></table></div></section>{preview && <Modal onClose={() => { URL.revokeObjectURL(preview); setPreview(null) }}><img className="receipt-preview" src={preview} alt="Purchase receipt" /></Modal>}</PageFrame> }
function ExpensesComposer({ notify }) { const [form, setForm] = useState({ item: '', quantity: 1, totalCost: '', purchaseDate: '', purpose: '' }); const [receipt, setReceipt] = useState(null); const [error, setError] = useState(''); const submit = async (event) => { event.preventDefault(); const body = new FormData(); Object.entries(form).forEach(([key, value]) => body.append(key, value)); body.append('receipt', receipt); try { await api('/reports/expenses', { method: 'POST', body }); notify('Purchase submitted'); setForm({ item: '', quantity: 1, totalCost: '', purchaseDate: '', purpose: '' }); setReceipt(null) } catch (e) { setError(e.message) } }; return <section className="surface"><SurfaceHeading eyebrow="Purchases / Expenses" title="New purchase" /><form onSubmit={submit} className="report-form"><label>Item / purchase name<input required value={form.item} onChange={(e) => setForm({ ...form, item: e.target.value })} /></label><label>Quantity<input required min="1" type="number" value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} /></label><label>Total cost<input required min="0" type="number" value={form.totalCost} onChange={(e) => setForm({ ...form, totalCost: e.target.value })} /></label><label>Date<input required type="date" value={form.purchaseDate} onChange={(e) => setForm({ ...form, purchaseDate: e.target.value })} /></label><label className="full-field">Purpose / description<textarea required value={form.purpose} onChange={(e) => setForm({ ...form, purpose: e.target.value })} /></label><label className="full-field">Receipt image<input required type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => setReceipt(e.target.files[0])} /></label><ErrorMessage message={error} /><Button type="submit">Submit purchase</Button></form></section> }
